...
async function loadMovies(searchTerm) {
    await fetch(`https://omdbapi.com/?s=${searchTerm}&page=1&apikey=25316a18`)
...
function displayMovieList(movies) {
    ...
    for (let idx = 0; idx < movies.length; idx++) {
        ...
        fetch(`https://www.omdbapi.com/?i=${movies[idx].imdbID}&apikey=25316a18`)
            .then((response) => response.json())
            .then((data) => {
...
function loadMovieDetails() {
    ...
    searchListMovies.forEach(movie => {
        movie.addEventListener('click', () => {
            fetch(`https://www.omdbapi.com/?i=${movie.dataset.id}&apikey=25316a18`)
                .then((response) => response.json())
                .then((data) => {
...
function displayMovieDetails(details) {
    ...
    <div id = "movie-info">
	  ...
	  <p>Year: ${details.Year} &nbsp;<span class=" bg-yellow-500 p-1 rounded">Rating: ${details.Ratings[0].Value}</span> &nbsp;Released: ${details.Released}</p>
